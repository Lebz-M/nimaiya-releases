...
if [ "$OS" = "Darwin" ] && [ "$ARCH" = "arm64" ]; then ASSET="nimaiya-macos-arm64";
  ...
  echo "✖ no prebuilt binary for $OS/$ARCH yet (currently: macOS Apple Silicon)."
...
( cd "$TMP" && grep " $ASSET\$" SHA256SUMS | shasum -a 256 -c - ) || { echo "✖ CHECKSUM MISMATCH — nothing installed. The download is corrupt or tampered."; exit 1; }
mkdir -p "$DEST"
install -m 755 "$TMP/$ASSET" "$DEST/nimaiya"
echo "✔ installed: $DEST/nimaiya ($("$DEST/nimaiya" --version))"
